            ...
            if (Vector3.Distance(clickPos, temp.transform.translation) < 2.0f)
                {
                    ...
                    {
                        GameObject worker = new Jelly("jelly_blue", game1, new Vector3(temp.transform.translation.X + random.Next(-2, 2), 0, temp.transform.translation.Z + random.Next(-2, 2)), 0, 0, 0, 0.005f, true, "worker",0.6f);
                        game1.scene.AddSceneObject("worker_X" + i.ToString(), worker);
                        game1.scene.SceneObjects["worker_X" + i.ToString()].StartAnimationClip("2", 20, true);
